...
Статический лендинг. Экраны: герой → «тебе не нужно становиться другой» → «тобой управляет состояние» → узнавание себя → что изменится → методология → автор → результаты до/после → формат → кому не подходит → цена. На герое и в блоке оплаты — таймер до 20 сентября и «Осталось 20 мест». Все кнопки (класс `js-bot`) ведут в `@teacher_life_bot` с токеном трекинга.
